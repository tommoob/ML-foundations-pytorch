...
def load_yaml_as_box(filename: str) -> ConfigBox:
    """Load a YAML file as a python-box ConfigBox object.

    Args:
        filename: Path to YAML file to load.

    Returns: Loaded YAML as ConfigBox.

    """
    params = ConfigBox(yaml.safe_load(open(filename, encoding="utf-8")))
    return params
